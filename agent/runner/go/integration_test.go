...
func TestIntegrationMultiTurn(t *testing.T) {
	...
	var history []runner.RPCEvent
	history = append(history, runner.RPCEvent{Type: "user_message", Summary: "Remember this number: 42. Just say OK."})
	for evt := range ch {
		if evt.Err != nil {
			t.Fatalf("turn 1 error: %v", evt.Err)
		}
		turn1Text += evt.Text
		history = append(history, runner.TextDeltaToRPCEvent(evt.Text))
	}
	if turn1Text == "" {
	...
	history = append(history, runner.RPCEvent{Type: "user_message", Summary: "What number did I ask you to remember? Reply with just the number."})
