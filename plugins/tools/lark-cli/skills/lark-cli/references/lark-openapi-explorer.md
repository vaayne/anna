...
用 `curl` 获取顶层索引，找到与需求相关的模块文档链接：

```
curl -sL https://open.feishu.cn/llms.txt
...
用 `curl` 获取模块文档，找到具体 API 的文档链接：

```
curl -sL https://open.feishu.cn/llms-docs/zh-CN/llms-<module>.txt
  → 提取问题："找出与 <用户需求> 相关的 API 说明和文档链接"
```

### Step 4：获取 API 完整规范

用 `curl` 获取具体 API 文档，提取完整的调用规范：

```
curl -sL https://open.feishu.cn/document/server-docs/.../<api>.md
